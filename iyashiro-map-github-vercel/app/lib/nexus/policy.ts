...
function numberGate(
  ...
  if (value === null) {
    return { gateId, label, status: "unknown", value, threshold, explanation: "値が未入力のため判定できません。" };
  }
...
export function assessNexusProperty(rawInput: NexusPolicyInput): NexusPolicyAssessment {
  ...
  const gates: NexusPolicyGate[] = [
    ...
    numberGate(
      ...
      floor,
      "2階または3階",
      (value) => NEXUS_POLICY.allowedFloors.includes(value as 2 | 3),
      "希望する階数です。",
      "2階または3階ではありません。",
    ),
    numberGate(
      ...
      fixedRentPerSqm,
      "5,000円/㎡以下",
      (value) => value <= NEXUS_POLICY.fixedRentPerSqmMaxJPY,
      "広さに対する固定費が上限内です。",
      "広さに対する固定費が5,000円/㎡を超えています。",
    ),
  ];
  ...
  return {
    ...
    input: { ...rawInput, totalMonthlyFixedJPY: total, areaSquareMeters: area, floor, currentness, identity },
    fixedRentPerSqm,
    gates,
    policyPass,
    decisionState,
