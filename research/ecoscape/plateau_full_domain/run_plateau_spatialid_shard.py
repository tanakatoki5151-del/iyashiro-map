...
def add(rec,d,bycoord,acc,fill):
 ...
 return touched
def fact(cell,x,tc,fr,shard,receipt_sha):
...
def main():
 ...
 ordered=sorted(alltiles,key=lambda q:(q[1],q[0]));batches=[ordered[i:i+a.batch_size] for i in range(0,len(ordered),a.batch_size)];seen=set();fills=[];receipts=[];fail=0;apirows=0;touch=0;skipbbox=0
