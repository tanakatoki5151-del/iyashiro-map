...
CODES=[f'131{i:02d}' for i in range(1,24)]+[f'141{i:02d}' for i in range(1,19)]+[f'141{i:02d}' for i in range(31,38)]
assert len(CODES)==48
URL='https://api.plateauview.mlit.go.jp/datacatalog/citygml/'+','.join(CODES)
...
report={'requestUrl':'48_CANONICAL_MUNICIPALITY_CODES','httpStatus':r.status_code,'rawBytes':len(raw),'rawSha256':hashlib.sha256(raw).hexdigest(),'requestedMunicipalities':len(CODES),'cityObjects':len(objs),'buildingFiles':len(rows),'buildingBytes':sum(x['fileSize'] for x in rows),'buildingFeatures':sum(x['features'] for x in rows),'years':sorted({x['year'] for x in rows if x['year'] is not None}),'cityCodes':sorted({str(x['cityCode']) for x in rows if x['cityCode']}),'maxFileBytes':max([x['fileSize'] for x in rows] or [0])}
