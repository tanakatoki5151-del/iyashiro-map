...
historical execution-ground boundary. Public standard images are downloaded in
an ordinary browser session after accepting the displayed GSI terms.
"""
...
async def main() -> None:
    ...
    async with async_playwright() as pw:
        ...
        async def on_response(response):
            url = response.url
            if "/app/api/" in url:
                row = {"url": url, "status": response.status, "contentType": response.headers.get("content-type")}
                network_urls.append(row)
                if response.status == 200 and "json" in (response.headers.get("content-type") or ""):
                    try:
                        api_payloads.append({"url": url, "payload": await response.json()})
                    except Exception:
                        pass
        ...
        # Include all planning agencies because pre-war domestic photos may be available.
        ...
        summary = {
            ...
            "qualityRule": "The proxy point selects imagery only. Any Kozukappara boundary requires the 1884 1:5,000 map, official dimensions, archaeological context and independent review. Acquisition alone never changes scoring.",
        }
